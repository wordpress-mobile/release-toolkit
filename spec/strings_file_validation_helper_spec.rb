...
module Fastlane
  ...
  module Helper
    class StringsFileValidationHelper
      ...
      def self.find_duplicated_keys_using_state_machine(file:)
        all_keys = {}
        dup_keys = []
...
describe Fastlane::Helper::StringsFileValidationHelper do
  ...
  context 'when there are duplicated keys' do
    ...
    it 'returns them using state-machine solution' do
      input = File.join(test_data_dir, 'file-with-duplicated-keys.strings')

      expect(described_class.find_duplicated_keys_using_state_machine(file: input)).to match_array [
        'dup1', 'dup2', 'dup3', 'dup4',
        '\U0025 key', '\U0026 key',
        'key with \"%@\" character',
        'key with \"%@\" but diff translations',
