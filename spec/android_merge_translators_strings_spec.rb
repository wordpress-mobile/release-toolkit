...
describe Fastlane::Actions::AndroidMergeTranslatorsStringsAction do
    before do
        @test_folder_path = File.join(Dir.tmpdir(), "my_tests")
        create_test_folder
    end

    it 'checks no merge' do
        run_test("test-merge-android-nomerge.json")
    end

    it 'checks merge simple' do
        run_test("test-merge-android-merge-simple.json")
    end

    it 'checks merge overwrite' do
        run_test("test-merge-android-merge-overwrite.json")
    end

    it 'checks merge overwrite with double key in pending file' do
        run_test("test-merge-android-merge-overwrite-double.json")
    end

    it 'checks merge overwrite with fuzzy strings' do
        run_test("test-merge-android-merge-overwrite-fuzzy.json")
    end
...
def run_test(script)
    test_script = get_test_from_file(script)
    create_test_data(test_script)
    Fastlane::Actions::AndroidMergeTranslatorsStringsAction.run({strings_folder: @test_folder_path})
    expect(read_result_data(test_script)).to eq(test_script["result"]["content"])
end
...
def get_test_from_file(filename)
    filename = test_data_path_for("translations/#{filename}")
...
def create_test_data(test_script)
    test_script["test_data"].each do | test_file | 
        generate_test_file(test_file["file"], test_file["content"])
    end
end
